...
路由默认保持 `custom_only`。在总览或系统页开启 `account_first` 后，运行中的 Worker 会热更新，无需重启。决策会同时识别接口与模型：`POST /responses` 接受非直接图片模型，`POST /images/generations` 接受 `gpt-image-*` 直接图片模型。标准路径和 `/v1` 路径都会映射到 ChatGPT Codex 账号 base 下的对应接口；query、请求字节、账号鉴权、响应状态、headers 和响应字节均保持透明。`chat/completions`、未知路径和接口/模型不匹配会带着明确原因进入自定义提供商池。这保留了 OpenAI 官方对[直接选择 GPT Image 的 Image API 与由主模型调用图片工具的 Responses API](https://developers.openai.com/api/docs/guides/image-generation)的区分。账号模型预检与明确 429 回退缓冲上限为 8 MiB；更大的请求会在首次投递前以 `account_body_too_large` 进入自定义池，若最终自定义候选的映射/override 无法在上限内完成，则返回 413 且不向上游投递。账号明确返回 429 时可安全回退一次到模型感知的自定义池；账号网络失败和投递状态不确定时绝不重放。自定义提供商的模型优先级、映射、可用性、权重和冷却行为保持不变。

总览的路由预演直接复用上述决策链，不会在浏览器中猜测路由。它在精确模型之外接受 `operation=responses`、`operation=chat/completions` 或 `operation=images/generations`；省略时仍兼容默认 `responses`。符合条件的 `gpt-image-*` Image API 预演会选择 ChatGPT，并展示当前模型感知的自定义回退；`responses + gpt-image-*` 仍进入自定义池，因为它属于不同请求协议。已认证的只读接口为 `GET /api/v1/routing-preview?model=<精确模型名>&operation=<接口类型>`，只返回有界路由元数据。
