...
Capture 初始化会建立只包含列表元数据的覆盖索引，让列表及统计查询避免扫描正文溢出页。索引有一次性构建耗时和存储开销；已有运行实例需在更新后正常重新初始化 Capture 才能获得索引。只读查询绝不创建索引，旧数据库仍使用准确的兼容查询。记录与统计来自同一已提交读取快照，后续刷新可见新写入及删除，不使用可能过时的统计缓存。进入转发记录页不再请求无关的 Metrics 和 Token 热力图；这些数据在相应页面加载，且不阻塞工作区初始化。
