...
- 查看包含路由原因、提供商选择原因、请求/转发模型、会话归属、输入/输出/缓存 Token 与客户端中止的九列转发账本；可选查看明确采集且经过隐私筛查的请求头和正文，并分别控制元数据/详情 Capture；
...
路由默认保持 `custom_only`。在总览或系统页开启 `account_first` 后，运行中的 Worker 会热更新，无需重启。决策会同时识别接口与模型：`POST /responses` 接受非直接图片模型，`POST /images/generations` 和 `POST /images/edits` 只接受 `gpt-image-*` 直接图片模型。标准路径和 `/v1` 路径都会映射到 ChatGPT Codex 账号 base 下的对应接口；其中 `POST /images/edits` 与 `POST /v1/images/edits` 会映射到 `/backend-api/codex/images/edits`。query、请求字节、账号鉴权、响应状态、请求/响应 headers 和响应字节均保持透明。对于 `multipart/form-data` Image Edits，CRP 会在 8 MiB 有界预检内严格解析唯一、非文件型的 `model` 字段，不接受该字段使用二进制 Content-Type，且绝不把二进制 part 解码或重新编码。`chat/completions`、未知路径和接口/模型不匹配会带着明确原因进入自定义提供商池。这保留了 OpenAI 官方对[直接选择 GPT Image 的 Image API 与由主模型调用图片工具的 Responses API](https://developers.openai.com/api/docs/guides/image-generation)的区分。账号模型预检与明确 429 回退缓冲上限为 8 MiB。更大的请求会在账号零投递前以 `account_body_too_large` 进入不依赖模型的自定义路径；若需要按精确模型选择 Provider，或需要 mapping/override，则返回 413 且不向任何上游投递。已缓冲的 Responses 和 Image Generations 在账号明确返回 429 时可回退一次到模型感知的自定义池；Image Edits 一旦投递便绝不重放，其 429 会原样返回并启动账号冷却，只有后续请求才会进入自定义池，此时 mapping 或 override 只改写 multipart 的 `model` 字段。账号网络失败、超时和投递状态不确定时绝不重放。自定义提供商的模型优先级、映射、可用性、权重和冷却行为保持不变。

总览的路由预演直接复用上述决策链，不会在浏览器中猜测路由。它在精确模型之外接受 `operation=responses`、`operation=chat/completions`、`operation=images/generations` 或 `operation=images/edits`；省略时仍兼容默认 `responses`。符合条件的 `gpt-image-*` Image API 生成和编辑预演会选择 ChatGPT，并展示当前模型感知的自定义回退；`responses + gpt-image-*` 仍进入自定义池，因为它属于不同请求协议。已认证的只读接口为 `GET /api/v1/routing-preview?model=<精确模型名>&operation=<接口类型>`，只返回有界路由元数据。
...
`转发记录` 是由本地 Capture 数据库提供的隐私受限请求账本，并以 Supervisor 设置作为唯一运行时来源；旧版 standalone 配置不能覆盖这些设置。列表接口仍然只返回元数据，九个核心列依次为时间、请求路径、路由决策、结果、请求/实际模型、会话 ID、Provider、输入/输出/缓存输入 Token 和耗时。`routeReason` 解释账号/自定义分流原因（账号可用、接口不支持、额度、冷却、认证等），`providerSelectionReason` 则区分精确模型优先级、权重、运行时顺序平局、冷却恢复、唯一候选和前一提供商失败后的重试。精确映射与 Provider override 会显示为 `请求模型 → 转发模型`；模型名和决策原因均可搜索，旧记录保持为空。表格省略重复的本地地址前缀，选中记录后仍保留完整入口和目标地址。以 `/models` 结尾的 Codex 模型目录请求默认隐藏，也可以明确显示；过滤发生在摘要统计和游标分页之前。
