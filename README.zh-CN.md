...
路由默认保持 `custom_only`。在总览或系统页开启 `account_first` 后，运行中的 Worker 会热更新，无需重启。决策会先识别接口类型，再判断账号状态和模型：只有模型不是直接 Image API 模型的 `POST /responses` 与 `POST /v1/responses` 可以优先使用 ChatGPT 账号。`chat/completions`、`images/generations`、未知路径和直接 `gpt-image-*` 模型会带着明确原因进入自定义提供商池。这遵循 OpenAI 官方对需 API key 的 [Image API](https://developers.openai.com/api/docs/guides/image-generation) 与 Responses 图片工具的区分，以及 Codex 文档中“[通用 OpenAI API 调用继续使用 Platform API key](https://developers.openai.com/codex/auth)”的说明。启用的路由规则组中，每条规则可以让多个精确请求模型共享同一提供商顺序；图片和 Chat Completions 请求现在也会在首次投递前完成模型优先级判断。未命中的模型继续按权重和运行时顺序打破平局。模型映射后的可用性与健康冷却仍具有约束力。一旦请求可能已经送达，自定义 POST 就不会被自动回放；只有连接建立前失败的有界 Responses 请求可以在同一请求内安全转移。非 Responses 请求绝不会回放，预检/回放缓冲上限为 8 MiB。

总览的路由预演直接复用上述决策链，不会在浏览器中猜测路由。它在精确模型之外接受 `operation=responses`、`operation=chat/completions` 或 `operation=images/generations`；省略时仍兼容默认 `responses`。Worker 运行时，预演会通过私有 IPC 读取实时账号资格、接口能力、提供商冷却、模型可用性、调度顺序与最终自定义选择原因。ChatGPT 账号路径不支持某个接口或直接图片模型时，看板会明确说明。已认证的只读接口为 `GET /api/v1/routing-preview?model=<精确模型名>&operation=<接口类型>`，只返回有界路由元数据。
