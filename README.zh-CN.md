...
- 查看包含请求/转发模型、会话归属、输入/输出/缓存 Token 与客户端中止的八列转发账本；可选查看明确采集且经过隐私筛查的请求头和正文，并分别控制元数据/详情 Capture；
...
`转发记录` 是由本地 Capture 数据库提供的隐私受限请求账本，并以 Supervisor 设置作为唯一运行时来源；旧版 standalone 配置不能覆盖这些设置。列表接口仍然只返回元数据，八个核心列依次为时间、请求路径、结果、请求/实际模型、会话 ID、Provider、输入/输出/缓存输入 Token 和耗时。精确映射与 Provider override 会显示为 `请求模型 → 转发模型`；模型名可搜索，旧记录保持为空。表格省略重复的本地地址前缀，选中记录后仍保留完整入口和目标地址。以 `/models` 结尾的 Codex 模型目录请求默认隐藏，也可以明确显示；过滤发生在摘要统计和游标分页之前。
