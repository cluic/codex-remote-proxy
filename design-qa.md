...
- The density revision replaces stacked explanatory cards with two compact routing rails: one current primary path and, only when relevant, one dashed conditional fallback. The always-visible flow prioritizes request model, account decision, matched rule, model rewrite, and predicted outlet; Provider retry order is compressed into a single strip beneath it.
...
- The custom-only board renders at 1128 × 244 px and the account-first board with fallback renders at 1128 × 320 px. The focused test enforces that 320 px desktop ceiling. The 390 px board renders at 358 × 473 px, keeps each active rail horizontal, removes redundant request/account nodes from the long custom-only rail, and passes clipping, overlap, and document-overflow checks.
- Active connectors retain the route pulse animation and conditional connectors retain an amber dashed treatment. Reduced-motion preferences collapse animation duration through the existing global rule.
