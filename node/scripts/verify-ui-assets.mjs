...
const assetEntries = [...Object.values(manifest.assets), manifest.notFound];
...
for (const entry of assetEntries) {
  ...
  if (extension === ".html") {
    assert.deepEqual(htmlScriptHashes(source), [...entry.inlineScriptHashes].sort(), `${entry.file} CSP hashes are out of sync`);
  }
  if (extension === ".css" || extension === ".js" || extension === ".mjs") {
    const externalPattern = extension === ".css" ? cssExternalUrlPattern : javascriptExternalFetchPattern;
