...
function ProviderForm({
  ...
  return (
    <form id="provider-form" className="provider-form grid gap-5" onSubmit={submit} noValidate>
      ...
      {advanced ? (
        <div className="form-grid advanced-fields grid gap-4 rounded-xl border border-border bg-muted/30 p-4 sm:grid-cols-2">
          <Field
...
function SupportedModelsForm({
  ...
  return (
    <form id="provider-models-form" className="supported-models-form grid gap-5" onSubmit={submit} noValidate>
      {error ? <FormError>{error}</FormError> : null}
      <div className="model-catalog-config grid gap-4 rounded-xl border border-border bg-muted/25 p-4">
        <Field
        ...
        />
        <div className="model-default-policy flex items-center justify-between gap-4">
          <div>
      ...
      <div className="model-catalog-toolbar flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
        <div>
      ...
      <div className="model-add-row grid items-end gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
        <Field
      ...
      <div className="model-management-list grid max-h-[34rem] gap-1 overflow-y-auto rounded-xl border border-border" role="list" aria-label={t("providers.modelCatalog")}>
        {visibleEntries.map((entry) => (
          <div className="model-management-row grid min-w-0 items-center gap-3 border-b border-border/70 px-3 py-2.5 last:border-b-0 sm:grid-cols-[minmax(0,1fr)_auto_auto]" role="listitem" key={entry.id}>
            <div className="model-management-name min-w-0">
...
export function ProvidersPage({
  ...
  return (
    <div className="page-stack space-y-5" data-testid="page-providers">
      <PageHeader
      ...
      {providers.length === 0 ? (
        ...
        <section className="provider-grid grid min-w-0 gap-4 xl:grid-cols-2" aria-label={t("providers.title")}>
          {providers.map((provider) => {
            ...
            return (
              <article
                key={provider.id}
                className={cx(
                  "provider-card group flex min-w-0 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-shadow hover:shadow-md",
                  active && "provider-card-active border-primary/35"
                )}
                data-testid={`provider-card-${provider.id}`}
                data-state={active ? "active" : "idle"}
              >
                <header className="provider-card-header flex min-w-0 flex-wrap items-start gap-3 border-b border-border px-4 py-4">
                  <div className="provider-avatar grid size-9 shrink-0 place-items-center rounded-lg bg-primary text-sm font-bold text-primary-foreground" aria-hidden="true">{provider.name.slice(0, 1).toUpperCase()}</div>
                  <div className="provider-card-title min-w-0 flex-1">
                    <h2>{provider.name}</h2>
                ...
                <dl className="provider-card-facts grid grid-cols-1 gap-4 px-4 py-4 sm:grid-cols-2">
                  ...
                  <div className="provider-weight-fact">
                    ...
                    <dd>
                      <form
                        className="provider-weight-form flex items-center gap-2"
                        onSubmit={(event) => {
                          event.preventDefault();
                      ...
                      >
                        <input
                          className="h-8 min-w-0 flex-1 rounded-lg border border-input bg-background px-2 text-sm outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30"
                ...
                {!active && !eligible ? (
                  <p className="provider-card-reason mx-4 mb-3 rounded-lg bg-muted px-3 py-2 text-xs text-muted-foreground">
                    {provider.credentialConfigured ? testLabel(provider, t) : t("providers.credentialRequired")}
                  </p>
                ) : null}
                <footer className="provider-card-actions mt-auto flex flex-wrap items-center justify-end gap-2 border-t border-border px-4 py-3">
      ...
      >
        {selected ? (
          <div className="provider-detail-grid grid gap-4">
            {selected.id === activeProviderId ? (
            ...
            <section className="catalog-section overflow-hidden rounded-xl border border-border">
              <div className="catalog-heading flex flex-wrap items-start justify-between gap-3 border-b border-border bg-muted/25 px-4 py-3">
                <div>
              ...
              {catalog?.entries.length ? (
                <ul className="model-list grid divide-y divide-border">{catalog.entries.slice(0, 12).map((item) => (
                  <li className={cx("flex min-w-0 items-center justify-between gap-3 px-4 py-2.5", !item.enabled && "model-list-disabled opacity-60")} key={item.id}>
      ...
      >
        {selected ? (
          <div className="test-provider-form grid gap-4">
            <ModelPicker
              key={selected.id}
              id="test-model"
              model={model}
