...
function MappingForm({
  ...
  return (
    <form id={formId} className="mapping-form grid gap-5" onSubmit={submit} noValidate>
      ...
      />
      <div className="mapping-rule-editor overflow-hidden rounded-xl border border-border bg-card">
        ...
        <div className="mapping-rule-labels text-muted-foreground" aria-hidden="true">
          <span>{t("modelMappings.sourceModel")}</span>
          <span>{t("modelMappings.targetModel")}</span>
          <span />
        </div>
        <div className="mapping-rule-list grid gap-2 p-4">
          {rules.map((rule, index) => (
            <div className="mapping-rule-row grid items-center gap-2 rounded-lg border border-border bg-background p-2 sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)_auto]" key={rule.key}>
              <label>
                <span className="visually-hidden">{t("modelMappings.sourceModelNumber", { value: index + 1 })}</span>
                <input
                  className="h-9 w-full rounded-lg border border-input bg-background px-3 font-mono text-sm outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30"
...
export function ModelMappingsPage({
  ...
  return (
    <div className="page-stack mapping-page space-y-5" data-testid="page-model-mappings">
      <PageHeader
      ...
      {groups.length === 0 ? (
        ...
        <div className="mapping-workspace grid min-w-0 items-start gap-4 lg:grid-cols-[17.5rem_minmax(0,1fr)]">
          <section className="mapping-group-list overflow-hidden rounded-xl border border-border bg-card shadow-sm" aria-label={t("modelMappings.groups")}>
              ...
              return (
                <button
                  ...
                  className={cx(
                    ...
                    active && "mapping-group-item-active bg-accent text-accent-foreground"
                  )}
                  type="button"
                  aria-pressed={active}
                ...
                >
                  ...
                  <span>
                    ...
                    <small className="block text-xs text-muted-foreground">{t("modelMappings.groupSummary", {
          ...
          {selected ? (
            <section className="mapping-detail min-w-0 overflow-hidden rounded-xl border border-border bg-card shadow-sm" aria-labelledby="mapping-detail-title">
              <header className="mapping-detail-header flex flex-wrap items-start justify-between gap-4 border-b border-border px-5 py-4">
                ...
                <div className="mapping-detail-actions flex flex-wrap items-center gap-2">
                  <Button
                    disabled={readOnly || pending !== null}
              ...
              <div className="mapping-assignment-strip grid gap-3 border-b border-border bg-muted/35 px-5 py-4 sm:grid-cols-[auto_1fr]">
                <div>
                  <span>{t("modelMappings.assignedProviders")}</span>
                  <strong>{formatNumber(locale, assignedProviders.length)}</strong>
                </div>
                <div className="mapping-provider-tags flex flex-wrap items-center gap-2">
              ...
              <div className="mapping-rules-table-wrap table-scroll overflow-x-auto">
                <table className="mapping-rules-table w-full min-w-[34rem] border-collapse text-sm">
                  <caption className="visually-hidden">{selected.name}</caption>
                  <thead>
                    <tr>
