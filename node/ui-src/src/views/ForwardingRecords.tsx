...
function DataPayload({
  ...
  return (
    <article className="record-data-pane min-w-0 overflow-hidden rounded-xl border border-border bg-card" data-testid={`forwarding-${side}-data`}>
      <header className="record-data-heading flex flex-wrap items-center justify-between gap-3 border-b border-border bg-muted/25 px-4 py-3">
      ...
      {!available ? (
        <div className="record-data-empty grid min-h-48 place-items-center content-center gap-2 p-6 text-center text-muted-foreground">
        ...
        <>
          <div className="record-payload-meta flex flex-wrap items-center gap-3 border-b border-border px-4 py-2.5 text-xs text-muted-foreground" aria-label={t("forwarding.payloadInfo")}>
          ...
          <details className="record-header-details border-b border-border">
            <summary>
            ...
            {headerEntries.length > 0 ? (
              <dl className="record-headers grid gap-2 p-4">
          ...
          <section className="record-body-section min-w-0 p-4">
            <div className="record-body-heading flex flex-wrap items-center justify-between gap-3">
            ...
            {visibleBody ? (
              <pre className={cx("record-body min-w-0 overflow-auto rounded-lg bg-muted/50 p-4 font-mono text-xs", !formatted && "record-body-raw", expanded && "is-expanded")}>
...
function RecordSummary({
  ...
  return (
    <div className="record-summary-tab grid gap-4" data-testid="forwarding-detail-summary">
      <section className={cx("record-outcome-summary grid gap-3 rounded-xl border border-border bg-muted/25 p-4", `is-${record.outcome}`)}>
      ...
      <dl className="record-detail-facts grid gap-px overflow-hidden rounded-xl border border-border bg-border sm:grid-cols-2">
  ...
  return (
    <Sheet open onOpenChange={(open) => { if (!open) onClose(); }}>
      <SheetContent
        ...
        className="record-detail-dialog record-detail-drawer flex w-screen min-w-0 max-w-none flex-col overflow-hidden border-l border-border bg-background text-foreground shadow-2xl sm:w-[min(52.5rem,calc(100vw-2rem))]"
      ...
      >
        <header className="record-details-header flex flex-wrap items-start justify-between gap-4 border-b border-border px-5 py-4">
          <div>
        ...
        <div className="record-detail-nav flex flex-wrap items-center justify-between gap-3 border-b border-border bg-muted/25 px-4 py-2.5">
        ...
        <div className="record-detail-scroll min-h-0 flex-1 overflow-y-auto p-4">
        ...
        <p className="record-privacy-note flex min-h-10 shrink-0 items-center gap-2 border-t border-border bg-muted/35 px-4 py-2 text-xs text-muted-foreground"><LockKeyhole aria-hidden="true" />{t("forwarding.metadataOnly")}</p>
      </section>
      </SheetContent>
    </Sheet>
  );
      ...
      <Panel className="records-panel">
        ...
        {records.length > 0 ? (
          <>
            ...
            <div className="records-mobile-list gap-3 p-3" data-testid="forwarding-mobile-list">
              {records.map((record) => {
                const startedAt = recordTimeParts(locale, record.startedAt);
                const models = modelValues(record);
                return (
