...
export function ActivityPage({ locale, t, data, providers, loading, onPage }: ActivityProps) {
  ...
  return (
    <div className="page-stack space-y-5" data-testid="page-activity">
      ...
      {data.events.length === 0 ? (
        ...
        <section className="activity-panel overflow-hidden rounded-xl border border-border bg-card shadow-sm" aria-label={t("activity.title")}>
          <div className="activity-table-heading border-b border-border bg-muted/35 px-4 py-2.5 text-xs text-muted-foreground" aria-hidden="true">
            <span>{t("common.time")}</span>
            <span>{t("activity.action")}</span>
            <span>{t("activity.category")}</span>
            <span>{t("activity.result")}</span>
          </div>
          <div className="activity-list divide-y divide-border">
            {data.events.map((event, index) => {
              ...
              return (
                <details className="activity-event group" key={`${event.timestamp ?? "event"}-${index}`}>
                  <summary className="cursor-pointer px-4 py-3 transition-colors hover:bg-muted/35">
                  ...
                  <div className="activity-detail border-t border-border bg-muted/20 px-4 py-3">
                    ...
                    {Object.keys(event.details).length > 0 ? (
                      <dl className="activity-detail-list grid gap-2 sm:grid-cols-2">
      ...
      {data.events.length > 0 ? (
        <nav className="pagination flex items-center justify-between gap-3 rounded-xl border border-border bg-card p-3 shadow-sm" aria-label={t("activity.title")}>
          <Button
