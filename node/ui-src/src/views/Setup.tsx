...
export function SetupPage({
  ...
  return (
    <div className="page-stack setup-page space-y-5" data-testid="page-setup">
      <PageHeader title={t("setup.title")} subtitle={t("setup.subtitle")} />
      <div className="setup-layout grid min-w-0 items-start gap-4 lg:grid-cols-[17.5rem_minmax(0,1fr)]">
        <aside className="setup-progress overflow-hidden rounded-xl border border-border bg-primary p-5 text-primary-foreground shadow-sm" aria-label={t("setup.progress")}>
          ...
          <ol>
            {steps.map((step, index) => (
              <li
                key={step.key}
                aria-current={phase === step.key ? "step" : undefined}
                className={cx(
                  "flex items-center gap-3 rounded-lg px-3 py-2.5",
                  step.complete && "step-complete",
                  phase === step.key && "step-current bg-primary-foreground/10"
                )}
              >
        ...
        <section className="setup-stage min-w-0 overflow-hidden rounded-xl border border-border bg-card p-5 shadow-sm sm:p-6">
          {phase === "provider" ? (
            <form className="grid gap-5" onSubmit={createProvider} noValidate>
              <header className="setup-stage-header border-b border-border pb-4">
              ...
              <div className="setup-stage-actions flex justify-end border-t border-border pt-4">
          ...
          {phase === "test" || phase === "activate" ? (
            <div className="grid gap-5">
              ...
              <div className="setup-test-controls grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
                <ModelPicker
                  key={selectedId}
                  id="setup-test-model"
                  model={model}
          ...
          {phase === "codex" ? (
            <div className="grid gap-5">
              <header className="setup-stage-header border-b border-border pb-4"><h2>{t("setup.codexTitle")}</h2><p>{t("setup.codexHelp")}</p></header>
          ...
          {phase === "worker" ? (
            <div className="grid gap-5">
              <header className="setup-stage-header border-b border-border pb-4"><h2>{t("setup.workerTitle")}</h2><p>{t("setup.workerHelp")}</p></header>
          ...
          {phase === "complete" ? (
            <div className="setup-complete grid min-h-80 place-items-center content-center gap-3 text-center">
