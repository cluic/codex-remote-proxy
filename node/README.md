...
The UI can create, test, prioritize, prefer, hot-update, and delete named providers; create reusable exact model-mapping groups; assign multiple exact requested models to each provider-order rule; configure provider model-discovery paths; and add, remove, enable, or disable individual model entries. Overview previews a selected `responses`, `chat/completions`, or `images/generations` operation plus exact model through the same live account-capability gate, routing rule, provider order, rewrite, predicted outlet, and conditional fallback used by proxy traffic. Only eligible Responses requests use ChatGPT subscription-account routing; direct Image API operations/models and Chat Completions are explicitly custom. It can also manage client API keys, Worker listening/lifecycle, ChatGPT account state, Metrics, Forwarding Records, Activity, autostart, and diagnostics. A provider must pass an OpenAI Responses compatibility test before it enters the runtime pool. Explicit model-set priority is evaluated before weight ordering for every preflight-inspectable operation, while health cooldowns and provider model controls remove ineligible candidates. Provider, model-control, mapping, routing-rule, preference, and weight changes atomically hot-apply a newer confirmed Worker snapshot.
