...
Unsafe legacy paths, invalid existing registries, permission failures, and identity races remain fail-closed. Multi-credential rollback deletes transaction-owned references in reverse order, restores only sources modified in the attempt, and retains both canonical locks if cleanup degrades. Post-commit Activity failures report committed degradation.
