...
> Release status: npm `0.4.2` is current. Changes after `0.4.2` remain unreleased until their deterministic, platform, and human-review gates pass. Ordinary updates use patch releases.
...
The authenticated Admin contract exposes the sanitized account projection through `GET /api/v1/status`, refreshes it through `POST /api/v1/account/refresh`, lists bounded metadata through `GET /api/v1/forwarding-records`, updates weights through `PATCH /api/v1/providers/:id/weight`, and accepts one of `routingMode`, `captureEnabled`, or `autoStartEnabled` at `PATCH /api/v1/settings`. Every mutation retains the existing same-origin session and CSRF requirements.

`Forwarding Records` is a complete metadata-only route over the local Capture database with filters, bounded search, keyset pagination, summary counts, details, and a Capture toggle. New rows persist final route/provider attribution; legacy rows use best-effort URL inference. The query selects no request/response bodies or authorization headers. Overview Metrics remains an independent anonymous aggregation using fixed UTC hourly buckets, semantic Responses completion, unavailable-rate disclosure after dropped observations, and explicit grouped Provider/model remainders.
...
The first supervisor startup transactionally reads legacy `config.json` and `node/proxy-config.json` when present, writes collision-safe byte-exact private backups, stores the secret through the required native adapter, creates schema-4 `providers.json` in `custom_only` mode with one inactive, untested, weight-100 `Default` profile, validates the registry, and only then scrubs legacy secret fields. The user must test and prefer `Default`; migration does not assume compatibility. Valid schema-2 and schema-3 registries are independently backed up and upgraded atomically; existing routing/Capture state is preserved and providers receive weight `100`.
