...
Overview pairs the 24-hour/7-day operational Metrics with a GitHub-style daily Token heatmap. The heatmap uses a separate compact 84-day UTC rollup, exposes exact input/output totals and request coverage through pointer, click, and keyboard selection, and distinguishes unobserved or partial usage from true zero-traffic days. Hourly and daily API failures are isolated; a degraded daily store keeps rendering its latest in-memory totals with a warning. Reliability excludes client aborts; model rows are expandable and separate unknown/grouped traffic; the provider table displays every bounded row returned by the API.

The UI can create, test, prioritize, prefer, hot-update, and delete named providers; create reusable exact model-mapping groups; assign multiple exact requested models to each provider-order rule; configure provider model-discovery paths; and add, remove, enable, or disable individual model entries. Overview previews a selected operation, exact model, and Image Edits request format through the same live account-capability gate used by proxy traffic. Image Edits offers JSON, multipart, and unsupported-format diagnostics; the selected `requestFormat` travels through Admin and Worker IPC instead of being reconstructed in the browser. The board also exposes routing rules, provider order, rewrites, predicted outlets, and conditional fallbacks.
...
Overview projects the Codex account authentication mode, plan, and only the normalized quota windows returned by the private app-server protocol; System keeps a compact account/routing status. Routing defaults to `custom_only`; `account_first` hot-applies and covers Responses, GET `/models` and `/v1/models`, Image Generations, and Image Edits when the operation, format, model, account, and quota gates are eligible. Account model catalogs map to `/backend-api/codex/models`, preserve the query string, and safely fall back to the custom Provider pool after an account `429` before downstream delivery. Image Edits maps to `/backend-api/codex/images/edits` and supports both bounded `application/json` Codex payloads and standard `multipart/form-data`. JSON inspection reads only the top-level model and forwards every byte unchanged; multipart inspection parses only the bounded textual model field and never decodes binary parts. Unsupported formats fail with 415, while missing models and invalid multipart fail with distinct 400 reasons before any account or custom delivery. Image Edits 429, timeout, and uncertain delivery are never replayed. Account headers are stripped from every custom request, and custom credentials/extra headers never cross into the account request.

The authenticated Admin contract exposes the sanitized account projection through `GET /api/v1/status`, refreshes it through `POST /api/v1/account/refresh`, and previews bounded routing metadata through `GET /api/v1/routing-preview?model=<exact-model>&operation=<operation>&requestFormat=<format>`. The remaining Provider, routing-rule, model-mapping, Capture, access-key, and settings contracts retain their existing authentication, same-origin session, CSRF, and bounded-field requirements.

Route Preview remains limited to model-bearing inference operations. `/models` scheduling is evaluated only for live GET traffic because catalog requests do not contain a request model.

`Forwarding Records` uses a compact metadata-only desktop list, narrow-screen record cards, and a selected-record dialog that preserves the list position. Outcome filters, bounded search, keyset pagination and default exclusion of `/models` remain. HTTP status and final outcome are distinct; details include safe recorded errors, routing reasons, full URLs and IDs. The authenticated `GET /api/v1/forwarding-records/:id` route is issued only after explicit selection, sets `Cache-Control: no-store`, and preserves nested `request.body`/`response.body` payloads. Switching records clears prior payloads before loading the next.

The list API also accepts canonical UTC `since`/`until` timestamps (`YYYY-MM-DDTHH:mm:ss.sssZ`, inclusive start/exclusive end), exact requested-or-forwarded `model`, persisted `providerId` and `sessionId`. Exact text filters are nonempty, trimmed, control-free and bounded to 256 Unicode code points. Facets apply all these filters plus search and model-request visibility, ignoring only outcome/paging; absent legacy exact fields never match. Refresh retains the last successful list. Capture writer initialization creates a metadata-only covering index to avoid body scans; existing running installations require normal Capture reinitialization after updating. The read-only service never installs indexes, keeps legacy results exact, and reads rows and counts from one committed snapshot. No stale count cache is used. Workspace startup no longer waits for Metrics; Forwarding does not request unrelated charts.

For nonempty existing databases, index preparation runs in an isolated background thread rather than on the gateway startup/IPC path. Capture reports `enabling` and remains inactive during preparation: proxy traffic continues, new requests are temporarily not recorded, and existing records remain intact. Successful preparation automatically activates recording; the UI discloses this gap and observes completion. Disable/path changes/shutdown invalidate pending work and wait for confirmed termination before replacement; normal Worker shutdown awaits background resource settlement. A fully verified successful Worker retry clears an earlier startup error. Empty databases initialize the covering index immediately, and read-only history queries keep their legacy fallback while preparation is pending.

Metadata Capture and detail Capture are separate Supervisor settings. Details default off, cannot be enabled while metadata Capture is off, and are cleared by disabling metadata Capture. The effective choice is snapshotted when each request begins, so an in-flight request cannot change storage mode halfway through. New rows also persist bounded cached-input Tokens. Sensitive headers are redacted both before storage and before public projection. Overview Metrics remains an independent anonymous aggregation using fixed UTC hourly buckets for 24-hour/7-day summaries and compact UTC daily buckets for the 12-week Token heatmap. Schema 1 Metrics files remain readable and reconstruct the available daily history from their retained hourly buckets before the next schema 2 write.
...
Pass-through traffic streams with backpressure and preserves request encodings byte-for-byte. JSON model override is an 8 MiB bounded transformation that preserves supported content encodings and strips stale integrity headers only after a rewrite. Image Edits preflight derives its parser from Content-Type rather than the endpoint alone: JSON reads the top-level `model` without rewriting image URLs or Base64 fields, while multipart requires exactly one textual non-file model field and never decodes binary parts. Downstream cancellation destroys the corresponding upstream work.

With detail Capture enabled, Capture stores at most 1 MiB per request and response body and retains total observed byte counts. With details off, headers/body content remain empty while metadata and byte totals are preserved. With configured protected values, truncated bodies, declared or detected compressed bodies, and bodies containing literal or recoverably encoded protected values are stored as `empty-truncated`; fully screened text/binary records retain explicit UTF-8/base64 encoding. Configured API keys and extra-header values are redacted from Capture headers, bodies, URL/ID metadata, detail responses, and debug logs. Independent buffered Metrics inspection is bounded to 8 MiB. Headerless and declared SSE are inspected incrementally with bounded events, Responses success requires semantic completion rather than HTTP 2xx alone, and observed usage is persisted without enabling detail Capture.
...
The first supervisor startup parses legacy `config.json` and `node/proxy-config.json` as independent Provider candidates. One complete source becomes an inactive, untested `Default`; canonical duplicates collapse to one candidate; conflicting complete sources become ordered `Recovered runtime` and `Recovered saved` Providers with independent native credential references and no automatic activation. Parseable sources are byte-backed-up before credential/registry writes and scrubbed only after schema-9 validation. Malformed sources remain unchanged. If no complete candidate exists, the Supervisor continues with an empty in-memory registry so Web Setup can collect a new Provider without filesystem repair. Valid schema-2 through schema-8 registries retain their strict atomic upgrade path.

Unsafe legacy or registry paths, permission failures, and identity races remain fail-closed. Multi-credential rollback deletes transaction-owned references in reverse order, restores only sources modified in the attempt, and retains both canonical locks if cleanup degrades. Post-commit Activity failures report committed degradation.

A safely readable regular `providers.json` whose JSON/schema/document is invalid is byte-backed-up and quarantined through a fixed no-replace `providers.json.recovery-invalid` hard-link marker before Setup starts with an empty in-memory registry. A private three-phase pending journal binds the source inode and both PID/transaction lock identities, allowing dead-process recovery before ordinary lock acquisition. Unsafe paths, foreign markers, mixed foreign locks, and identity mismatches are preserved and remain blocking.
