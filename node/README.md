...
`Forwarding Records` uses a metadata-only list contract with success/rejected/error/aborted filters, bounded search, keyset pagination, summary counts, and default exclusion of paths ending in `/models`. Its nine columns cover time, request, route decision, result, requested/effective model, session ID, Provider, input/output/cached-input Tokens, and duration. New nullable `routeReason` and `providerSelectionReason` fields explain the account gate and final custom scheduler choice; legacy rows remain readable with null reasons. Full origins remain out of the table but are retained in the selected record metadata. The authenticated `GET /api/v1/forwarding-records/:id` detail route is issued only after explicit row selection, sets `Cache-Control: no-store`, and returns nested `request`/`response` payloads only when that record was captured with details enabled.
