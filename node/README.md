...
`Forwarding Records` is a complete metadata-only route over the local Capture database with success/rejected/error/aborted filters, bounded search, keyset pagination, summary counts, details, and a Capture toggle. Capture schema 5 adds nullable `requested_model` and `forwarded_model` columns; new rows persist both values alongside final route/provider attribution and observed input/output Tokens. Exact mappings and Provider overrides therefore remain explainable without exposing request bodies, model search covers both columns, and legacy rows remain nullable. The UI requests `includeModels=false` by default, so incoming paths ending in `/models` are excluded before pagination and summary aggregation until the user enables the model-request toggle; API clients that omit the parameter retain the backward-compatible inclusive behavior. Table cells reduce full incoming URLs to path/query text while details keep the full endpoints. A close after observed semantic completion is successful, while a true pre-completion client close is aborted rather than a provider error. Legacy rows use best-effort URL inference. The query selects no request/response bodies or authorization headers. Overview Metrics remains an independent anonymous aggregation using fixed UTC hourly buckets for 24-hour/7-day summaries and compact UTC daily buckets for the 12-week Token heatmap. Schema 1 files remain readable and reconstruct the available daily history from their retained hourly buckets before the next schema 2 write. The authenticated heatmap contract is `GET /api/v1/metrics/token-heatmap?window=12w`; it projects only daily request counts and observed input/output Token totals, with no Provider/model dimensions or request content.
