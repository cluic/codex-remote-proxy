...
The UI can create, test, prioritize, prefer, hot-update, and delete named providers; create reusable exact model-mapping groups; assign multiple exact requested models to each provider-order rule; configure provider model-discovery paths; and add, remove, enable, or disable individual model entries. Overview previews a selected `responses`, `chat/completions`, `images/generations`, or `images/edits` operation plus exact model through the same live account-capability gate used by proxy traffic. Eligible Responses and direct Image API requests use the ChatGPT Codex account base with their operation-specific paths; Chat Completions and operation/model mismatches remain custom. The board also exposes routing rules, provider order, rewrites, predicted outlets, and conditional fallbacks. Explicit model-set priority is evaluated before weight ordering for every preflight-inspectable custom operation, while health cooldowns and provider model controls remove ineligible candidates.
...
Pass-through traffic streams with backpressure and preserves request encodings byte-for-byte. JSON model override is an 8 MiB bounded transformation that preserves gzip, deflate, Brotli, and native zstd encoding when available, strips stale integrity/signature headers after a rewrite, and falls back to identity for a verified single-frame zstd request when Node has no native zstd compressor. Image Edits multipart inspection is separately bounded at 8 MiB, requires exactly one strictly parsed non-file `model` field with no binary content type for account eligibility, and never decodes or re-encodes binary parts; a custom mapping or override rewrites only that field. Downstream cancellation destroys the corresponding upstream work.
