...
| 2026-07-14 | Require deterministic composition and a separate real live smoke | Module tests or loopback/injected integration cannot prove native keyring and external-provider behavior | A core completion claim requires both gates plus redacted cleanup evidence |
| 2026-07-14 | Retain `provider add --api-key <KEY>` for this slice | The user deferred credential-input redesign | Record argv/history exposure as a known risk and do not claim it is resolved |
