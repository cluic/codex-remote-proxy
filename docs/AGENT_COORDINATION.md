...
| --- | --- | --- | --- | --- | --- |
| Core-first D1 full production chain | Unassigned | Expand the minimum clean-home GREEN test into the serial production-component A/B switching, restart, secret-scan, and cleanup gate | `node/test/integration/core-real-chain.test.mjs` is available and no longer frozen; any test-runner files require explicit registration | Pending; Scope A made the retained minimum RED condition GREEN, but the complete production-chain extension has not started | No writer authority until assigned; Scope A/B files, all `docs/**`, Web/E2E, manifests, package tests, `output/**`, and every unregistered file remain no-edit |

D1's minimum RED evidence remains retained and Scope A has made that condition GREEN. This does not complete D1: `node/test/integration/core-real-chain.test.mjs` is no longer frozen, but the full production-chain extension remains pending with no assigned owner.
...
The approved slice is specified in `docs/superpowers/specs/2026-07-14-crp-core-first-cli-design.md` and planned in `docs/superpowers/plans/2026-07-14-crp-core-first-cli-implementation.md`. Scope A and Scope B are completed with review and test evidence recorded above. The minimum D1 condition is GREEN, but its full production-chain extension remains pending and unassigned; D2 follows only after complete D1 GREEN. Remote platform/native/visual/migration evidence, L3 approval, push, pull request, merge, versioning, publishing, and release remain outstanding; Task 12 is parked and incomplete.
