...
| --- | --- | --- | --- | --- | --- |
| Core-first Scope A: Codex bootstrap | Unassigned | TDD for missing-config creation, races/cleanup, and stable public bootstrap errors | `node/src/codex/codex-config.mjs`; `node/src/supervisor/supervisor.mjs`; `node/test/codex-config.test.mjs`; `node/test/integration/admin-server.test.mjs` | Planned; assign one writer before editing | All Scope B files; D1/D2 gate files; `node/ui/**`; `node/test/e2e/**`; `output/**`; manifests; package allowlist; contracts |
| Core-first Scope B: CLI contract | Unassigned | TDD for locale precedence, complete `en`/`zh-CN` human output, stable JSON failures, and staged start | `node/bin/crp.mjs`; `node/test/crp.test.mjs`; new `node/test/crp-i18n.test.mjs` | Planned; assign one writer before editing | All Scope A files; D1/D2 gate files; `node/ui/**`; `node/test/e2e/**`; `output/**`; manifests; package allowlist; contracts |

Before either implementation writer starts, the coordinator must create and run only the minimal `node/test/integration/core-cli-chain.test.mjs` clean-home RED skeleton, retain its safe failure evidence, and release that file. Scope A/B then run with D1 frozen; after both close, the coordinator resumes D1 for full-chain completion, serial runner wiring, and GREEN in the same working tree, followed by D2.
