...
- scan a nine-column Forwarding Records ledger with route reasons, provider-selection reasons, requested/forwarded models, input/output/cached Tokens, session attribution, and client aborts; optionally inspect explicitly captured privacy-screened headers and bodies; and control metadata/detail Capture independently;
...
Routing defaults to `custom_only`. The Overview and System toggles can hot-apply `account_first` without restarting a running Worker. The decision classifies operation and model together: `POST /responses` accepts non-Image-API models, while `POST /images/generations` and `POST /images/edits` accept only direct `gpt-image-*` models. Both canonical and `/v1` request paths map to the corresponding path under the ChatGPT Codex account base; in particular, `POST /images/edits` and `POST /v1/images/edits` map to `/backend-api/codex/images/edits`. Query, request bytes, account authorization, response status, request/response headers, and response bytes remain transparent. For `multipart/form-data` Image Edits, CRP strictly parses the sole non-file `model` field within an 8 MiB bounded preflight, accepts no binary content type for that field, and never decodes or re-encodes binary parts. `chat/completions`, unknown paths, and operation/model mismatches enter the custom-provider pool with an explicit reason. This preserves the official OpenAI distinction between choosing a GPT Image model through the [Image API](https://developers.openai.com/api/docs/guides/image-generation) and using a mainline Responses model with the image-generation tool. CRP uses the account only when Codex reports a signed-in ChatGPT session, a unique account identity, and available quota. Account model preflight and explicit-429 replay buffering are capped at 8 MiB. A larger request enters an unrestricted custom route before account delivery with `account_body_too_large`; if exact-model Provider selection or a mapping/override would be required, CRP returns 413 without delivering bytes to any upstream. Explicit account 429 responses for bounded Responses and Image Generations can fall through once to the model-aware custom pool. Image Edits are never replayed after delivery: their 429 response is returned and starts account cooldown, so only a later request can enter custom, where a mapping or override rewrites only the multipart `model` field. Account network failures, timeouts, and uncertain delivery are never replayed. Custom-provider model priority, mapping, availability, weight, and cooldown behavior remain unchanged.

The Overview route preview uses that same decision chain instead of reconstructing routing in the browser. It accepts `operation=responses`, `operation=chat/completions`, `operation=images/generations`, or `operation=images/edits` together with the exact model; omission preserves the compatible `responses` default. Eligible `gpt-image-*` Image API generation and edit previews select ChatGPT and expose the current model-aware custom fallback, while Responses with a direct image model remain custom because that is a different request contract. The authenticated read-only contract is `GET /api/v1/routing-preview?model=<exact-model>&operation=<operation>` and returns bounded routing metadata only.
...
`Forwarding Records` is a privacy-bounded request ledger backed by the local Capture database. Supervisor settings are its single runtime source; the legacy standalone config cannot override them. The list route remains metadata-only and shows nine scan columns: time, request path, route decision, result, requested/effective model, session ID, Provider, input/output/cached-input Tokens, and duration. `routeReason` explains the account/custom gate (`account_eligible`, `unsupported_operation`, quota, cooldown, auth, and related states), while `providerSelectionReason` distinguishes exact model priority, weight, runtime-order ties, cooldown recovery, sole eligibility, and retry after a provider failure. Exact mappings and Provider overrides appear as `requested → forwarded`; model and decision names are searchable and legacy rows remain nullable. Repeated local origins are omitted from table paths, while the selected record retains complete incoming and target URLs. Codex model-catalog requests ending in `/models` are hidden by default and can be included explicitly; this filter runs before summary counts and keyset pagination.
