...
Routing defaults to `custom_only`. The Overview and System toggles can hot-apply `account_first` without restarting a running Worker. The decision classifies the operation before account state or model routing: only `POST /responses` and `POST /v1/responses` with a non-Image-API model are eligible for the ChatGPT account path. `chat/completions`, `images/generations`, unknown paths, and direct `gpt-image-*` models enter the custom-provider pool with an explicit reason. This follows the official OpenAI distinction between the API-key-authenticated [Image API](https://developers.openai.com/api/docs/guides/image-generation) and the Responses image-generation tool, plus the Codex guidance that [general OpenAI API calls continue to use Platform API keys](https://developers.openai.com/codex/auth). CRP uses the account only when Codex reports a signed-in ChatGPT session, a unique account identity, and available quota. Each rule in an active routing-rule group can assign multiple exact requested models to one provider order; this model-aware preflight now applies to image and chat-completions operations before the first custom delivery. Unmatched models retain weight and runtime-order tie-breaking. Provider model control is evaluated after that candidate's model mapping. Health cooldowns always take precedence. Explicit account rate limiting falls through to the custom pool. CRP does not replay a custom POST after delivery may have occurred; only a bounded Responses request that fails before the upstream connection is established can move to the next candidate in the same request. Non-Responses requests are never replayed, and preflight/replay buffering is capped at 8 MiB.

The Overview route preview uses that same decision chain instead of reconstructing routing in the browser. It accepts `operation=responses`, `operation=chat/completions`, or `operation=images/generations` together with the exact model; omission preserves the compatible `responses` default. While the Worker is running it reads live account eligibility, operation capability, provider cooldowns, model availability, scheduler order, and the final custom selection reason over private IPC. The board explicitly says when the ChatGPT account route does not support an operation or direct image model. The authenticated read-only contract is `GET /api/v1/routing-preview?model=<exact-model>&operation=<operation>` and returns bounded routing metadata only.
