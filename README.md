...
Routing defaults to `custom_only`. The Overview and System toggles can hot-apply `account_first` without restarting a running Worker. The decision classifies operation and model together: `POST /responses` accepts non-Image-API models, while `POST /images/generations` accepts direct `gpt-image-*` models. Both canonical and `/v1` request paths map to the corresponding path under the ChatGPT Codex account base; query, request bytes, account authorization, response status, headers, and response bytes remain transparent. `chat/completions`, unknown paths, and operation/model mismatches enter the custom-provider pool with an explicit reason. This preserves the official OpenAI distinction between choosing a GPT Image model through the [Image API](https://developers.openai.com/api/docs/guides/image-generation) and using a mainline Responses model with the image-generation tool. CRP uses the account only when Codex reports a signed-in ChatGPT session, a unique account identity, and available quota. Account model preflight and explicit-429 replay buffering are capped at 8 MiB; a larger request enters custom before first delivery with `account_body_too_large`, and any selected custom mapping/override that cannot be applied within the bound fails with 413 without upstream delivery. Explicit account 429 responses can fall through once to the model-aware custom pool; account network failures and uncertain delivery are never replayed. Custom-provider model priority, mapping, availability, weight, and cooldown behavior remain unchanged.

The Overview route preview uses that same decision chain instead of reconstructing routing in the browser. It accepts `operation=responses`, `operation=chat/completions`, or `operation=images/generations` together with the exact model; omission preserves the compatible `responses` default. Eligible `gpt-image-*` Image API previews select ChatGPT and expose the current model-aware custom fallback, while Responses with a direct image model remain custom because that is a different request contract. The authenticated read-only contract is `GET /api/v1/routing-preview?model=<exact-model>&operation=<operation>` and returns bounded routing metadata only.
