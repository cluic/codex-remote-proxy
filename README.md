...
- scan an eight-column Forwarding Records ledger with requested/forwarded models, input/output/cached Tokens, session attribution, and client aborts; optionally inspect explicitly captured privacy-screened headers and bodies; and control metadata/detail Capture independently;
...
`Forwarding Records` is a privacy-bounded request ledger backed by the local Capture database. Supervisor settings are its single runtime source; the legacy standalone config cannot override them. The list route remains metadata-only and shows eight scan columns: time, request path, result, requested/effective model, session ID, Provider, input/output/cached-input Tokens, and duration. Exact mappings and Provider overrides appear as `requested → forwarded`; model names are searchable and legacy rows remain nullable. Repeated local origins are omitted from table paths, while the selected record retains complete incoming and target URLs. Codex model-catalog requests ending in `/models` are hidden by default and can be included explicitly; this filter runs before summary counts and keyset pagination.
...
Migration reads the legacy `config.json` and runtime `node/proxy-config.json` when present. It creates collision-safe, byte-exact private backups, stores the credential through the required native backend, creates an inactive and untested schema-9 provider registry in `custom_only` mode with weight `100`, loopback listening, client-key authentication disabled, detailed Capture disabled, the default `/models` discovery path, newly discovered models enabled, and empty mapping/routing groups, validates the committed registry, and only then scrubs secret fields from the legacy files. Backups are retained. Schema-2 through schema-8 upgrades also retain byte-exact backups and restore the original bytes if validation or publication fails.
